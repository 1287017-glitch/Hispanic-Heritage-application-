...
st.markdown("""
    ...
    <div class="author-name">Siron Barker</div>
...
def calculate_batch_years(model, batch_years, df_clean, degree):
    """Calculate values for multiple years"""
    results = []
    for year in batch_years:
        result = calculate_custom_year_value(model, year, df_clean, degree)
        if result:
...
def create_simple_plot(df_clean, x_smooth, y_smooth, x_sample, y_sample, year_max, custom_calculation, batch_calculation):
    ...
    try:
        ...
        if batch_calculation:
            for calc in batch_calculation:
                color = 'green' if calc['is_interpolation'] else 'purple'
                marker = 'o' if calc['is_interpolation'] else '^'
                ax.scatter(calc['year'], calc['predicted_value'], 
                          color=color, s=100, marker=marker, zorder=6, edgecolors='black', linewidth=1.5)
...
try:
    ...
    # Define year range for calculations
    ...
    batch_calculation = None
    if calculate_batch and batch_years:
        batch_calculation = calculate_batch_years(model, batch_years, df_clean, degree)
    ...
    if batch_calculation:
        st.subheader("Batch Year Calculation Results")
        batch_df = pd.DataFrame([
            {
                'Year': calc['year'],
                'Predicted Value': f"{calc['predicted_value']:.2f}",
                'Lower Bound': f"{calc['lower_bound']:.2f}",
                'Upper Bound': f"{calc['upper_bound']:.2f}",
    ...
    if MATPLOTLIB_AVAILABLE:
        ...
        fig = create_simple_plot(df_clean, x_smooth, y_smooth, None, None, year_max, custom_calculation, batch_calculation)
        if fig:
            st.pyplot(fig)
        else:
